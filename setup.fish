...
if test -d /tmp/deps/anchor/spl
 ...
 sed -i '/\[dependencies\]/a spl-token-2022 = { git = "https://github.com/hamkj7hpo/token-2022.git", branch = "safe-pump-compat" }' Cargo.toml
 if grep -q "\[patch.crates-io\]" Cargo.toml
 sed -i '/\[patch.crates-io\]/,/^\[/ s|zeroize =.*||' Cargo.toml
 sed -i '/\[patch.crates-io\]/,/^\[/ {/^$/d}' Cargo.toml
 if ! grep -q "[^[:space:]]" Cargo.toml -A 1000 | grep -q "\[patch.crates-io\].*[^[:space:]]"
 ...
 git add Cargo.toml
 git commit -m "Remove duplicate spl-token-2022, remove zeroize patch, update dependencies" || true
 git push origin $branch || true
end
