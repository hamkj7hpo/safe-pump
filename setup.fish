...
# Fork and clone repositories with SSH
for repo in $repos
    set -l repo_dir $tmp_dir/$repo
    set -l origin_url https://github.com/$github_user/$repo.git
    set -l ssh_url git@github.com:$github_user/$repo.git

    echo "Processing $repo into $repo_dir..."
    if test -d $repo_dir
        echo "$repo_dir already exists, updating..."
        cd $repo_dir
        # Check if remote is already set to SSH
        ...
        git clone $ssh_url $repo_dir || continue
        cd $repo_dir
        git checkout $branch || git checkout -b $branch
    end
