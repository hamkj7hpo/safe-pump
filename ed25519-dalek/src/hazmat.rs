...
// Types used when the hazmat functions are enabled (re-exported at crate root)
...
impl Drop for ExpandedSecretKey {
    fn drop(&mut self) {
        // zeroize trait must be in scope to call `.zeroize()` on arrays + byte buffers
        // Scalar zeroization: convert to bytes, zeroize, and replace with zero scalar value
        let mut s_bytes = self.scalar.to_bytes();
        s_bytes.zeroize();
        // Replace scalar with zero scalar (from zeroed bytes)
        self.scalar = Scalar::from_bytes_mod_order([0u8; 32]);

        // zeroize hash_prefix array
...
impl ExpandedSecretKey {
    ...
    pub fn from_bytes(bytes: &[u8; 64]) -> Self {
        ...
        // clamp_integer mirrors RFC clamping rules and returns [u8;32] suitable for from_bytes_mod_order
